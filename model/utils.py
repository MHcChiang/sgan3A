...
def find_nan(variable, var_name):
    # Remove .data attribute (deprecated in modern PyTorch)
    # Use detach() if gradient tracking is not needed, otherwise use tensor directly
...
def get_total_norm(parameters, norm_type=2):
    ...
    if norm_type == float('inf'):
        total_norm = max(p.grad.abs().max() for p in parameters if p.grad is not None)
    else:
        total_norm = 0
        for p in parameters:
            try:
