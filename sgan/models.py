...
def get_noise(shape, noise_type, device=None):
    """
    Generate noise tensor with device-agnostic placement.
    
    Args:
        shape: Shape of the noise tensor
        noise_type: 'gaussian' or 'uniform'
        device: torch.device object or string ('cuda', 'cpu', etc.) or None for auto-detection
    ...
    if noise_type == 'gaussian':
        ...
        tensor = torch.rand(*shape).sub_(0.5).mul_(2.0)
    else:
        raise ValueError('Unrecognized noise type "%s"' % noise_type)
    
    # Handle device placement (device-agnostic for Mac/CPU compatibility)
    if device is not None:
        if isinstance(device, str):
            ...
            # If it's a tensor, extract its device
            device = device.device if hasattr(device, 'device') else torch.device('cpu')
        tensor = tensor.to(device)
    else:
        # Auto-detect: if CUDA available, use it; otherwise CPU (Mac-friendly)
...
class Encoder(nn.Module):
    ...
    def init_hidden(self, batch):
        """
        Initialize hidden state tensors on the same device as model parameters.
        Device-agnostic: works on CPU (Mac) and GPU.
        """
        # Get device from model parameters (works on any device)
        try:
            device = next(self.parameters()).device
        except:
            # Fallback to CPU if no parameters (shouldn't happen, but safe)
            device = torch.device('cpu')
    ...
    def forward(self, obs_traj):
        ...
        # Make contiguous before view (required for PyTorch 0.4.0 when tensor comes from permute)
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, user_noise=None):
        ...
        # Create decoder_c on same device as model (device-agnostic)
        try:
            device = next(self.parameters()).device
        except:
            device = torch.device('cpu')
